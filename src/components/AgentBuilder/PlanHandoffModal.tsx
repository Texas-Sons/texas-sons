...
interface ChosenDirection {
  name: string;
  seeds: { primaryColor: string; accentColor: string; fontFamily: string };
  rationale?: string;
  guidelines?: string;
...
export const PlanHandoffModal: React.FC<PlanHandoffModalProps> = ({
  ...
  /**
   * The Stitch direction that was picked, if one was.
   *
   * Folded into this spec rather than given its own "copy the design brief"
   * button, because two prompts describing the same project is how they drift.
   * The snapshot below is the live edited state, which is more current than the
   * intake row — so the direction is the only thing worth fetching.
   */
  ...
  useEffect(() => {
    ...
    fetch(`/api/stitch/variants/${encodeURIComponent(id)}`)
      ...
      .then((j) => {
        if (cancelled) return;
        const stored = j?.variants;
        const i = stored?.chosen;
        setChosenDirection(typeof i === 'number' ? stored.variants?.[i] ?? null : null);
      })
      .catch(() => {
        // No direction is a normal state, not a failure. The spec is complete
        // without one.
      });
    return () => {
      cancelled = true;
    };
  }, [isOpen, project.intakeId]);

  if (!isOpen) return null;

  const directionSection = chosenDirection
    ? `
---

## 5. Chosen Design Direction — ${chosenDirection.name}
Generated by Stitch and applied to this snapshot. The three values below are the
ones actually in force; everything after them is Stitch's reasoning, kept so the
intent survives the handoff.

- **Ground:** ${chosenDirection.seeds.primaryColor}
- **Accent:** ${chosenDirection.seeds.accentColor}
- **Font pairing:** ${chosenDirection.seeds.fontFamily}${
        chosenDirection.fonts?.heading
          ? `
- **Stitch asked for:** ${chosenDirection.fonts.heading}${chosenDirection.fonts.body ? ` over ${chosenDirection.fonts.body}` : ''} (mapped to the nearest pairing we load)`
          : ''
      }

${chosenDirection.rationale || ''}

${chosenDirection.guidelines ? `### Style guidelines from Stitch\n\n${chosenDirection.guidelines}` : ''}
`
    : '';
